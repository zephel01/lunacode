...
どのプロバイダーとモデルを選べばよいか迷ったときの判断基準です。

### プロバイダー選択

| 状況 | 推奨プロバイダー | 理由 |
|---|---|---|
| **最高品質・安定性重視** | `openai` (gpt-4o) | 最も信頼性が高く、複雑なタスクに強い |
| **コストを抑えたい** | `openai` (gpt-4o-mini) | 高品質で低コスト、日常的な用途に最適 |
| **プライバシー重視・オフライン** | `ollama` または `lmstudio` | データがローカルから外に出ない |
| **コーディング特化** | `zai` (glm-5.1) | GLM コーディング専用エンドポイント対応 |
| **既存の LLM サービスを統合** | `litellm` | 100+ プロバイダーへの統一インターフェース |

### Ollama モデル選択

ローカル実行の場合は、PC のスペックに合わせてモデルを選択してください。

| モデル | VRAM目安 | 用途 | 特徴 |
|---|---|---|---|
| `qwen2.5-coder:1.5b` | ~2GB | 軽量タスク・高速応答 | 最速。単純なコード補完向き |
| `qwen2.5-coder:7b` | ~8GB | バランス型（推奨） | 品質と速度のバランスが良い |
| `qwen2.5-coder:14b` | ~16GB | 高品質コーディング | コーディング特化で高精度 |
| `llama3.1:8b` | ~8GB | 汎用タスク | 日本語を含む汎用タスクに強い |
| `llama3.1:70b` | ~48GB | 最高品質ローカル | GPT-4 に近い品質（高スペック PC 向け） |
| `gemma3:4b` | ~4GB | 軽量・多言語 | Google 製。日本語対応が良好 |
| `mistral:7b` | ~8GB | 汎用・高速 | 軽量で高速なテキスト生成 |
| `deepseek-r1:8b` | ~8GB | 推論・分析 | 思考プロセスを明示した推論が得意 |

**モデル自動ルーティングを活用する場合**（Phase 4 機能）: `fast` と `heavy` の2モデルを設定しておくと、タスクの複雑度に応じて自動的に切り替わります。
...
| モデル | 用途 | コスト感 |
|---|---|---|
| `gpt-4o` | 最高品質・複雑なタスク | 高め |
| `gpt-4o-mini` | 日常的なタスク（推奨） | 低め |
| `o1-mini` | 数学・論理推論特化 | 中程度 |
