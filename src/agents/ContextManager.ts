...
export class ContextManager {
  ...
  fitMessages(messages: AgentMessage[]): AgentMessage[] {
    const totalTokens = TokenCounter.estimateMessages(messages as any[]);
    ...
    for (let i = otherMessages.length - 1; i >= 0; i--) {
      const tokens = TokenCounter.estimateMessages([otherMessages[i]] as any[]);
  ...
  getUsageInfo(messages: AgentMessage[]): {
    ...
    const used = TokenCounter.estimateMessages(messages as any[]);
    return {
      used,
