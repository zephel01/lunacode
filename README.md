<p align="center">
  ...
  <p align="center">
    ReAct パターン | ストリーミング | マルチエージェント並列 | サンドボックス隔離 | 自己評価ループ | MCP | マルチ LLM
  </p>
  <p align="center">
    ...
    <img alt="Tests: 780 pass" src="https://img.shields.io/badge/tests-780%20pass-brightgreen.svg" />
    <img alt="Phase: 30/31" src="https://img.shields.io/badge/phase-30%2F31%20complete-success.svg" />
  </p>
  <p align="center">
...
LunaCode は、Claude Code にインスパイアされたオープンソースの自律型コーディングエージェントです。ReAct パターンによるエージェントループ、長期ベクトルメモリ、マルチエージェントオーケストレーション、自己評価・自己修正ループ、タスク別モデルルーティング、MCP 統合、5社の LLM プロバイダー対応、構造化ログ（pino）、commander.js による CLI サブコマンド、SWE-bench 対応の Git ツール・マルチファイル編集・テスト実行ツールを統合し、自律稼働するコーディングアシスタントを目指しています。
...
- **15のコアツール** — Bash, ファイル読み書き・編集, マルチファイル一括編集, Glob, Grep, Git（status/diff/commit/apply/log）, テスト実行（pytest/bun/jest/go/cargo など）, delegate_task（リスクレベル管理付き）
...
- **マルチエージェント並列実行（Phase 31）** — `ParallelAgentCoordinator` で複数のトップレベル AgentLoop を独立 workspace 上で並行起動。`lunacode parallel` CLI から `--max-concurrency` / `--on-conflict` / `--timeout` / `--dry-run` 等で制御
...
- **サンドボックス Tier 1（Phase 24〜31）** — タスクごとに origin の隔離コピーを `apfs-clone / reflink / git-worktree / copy` のいずれかで作成。`ToolContext.basePath` 注入でツールの作用範囲を隔離。auto-merge の衝突検知（`abort / skip-conflicted / force`）。`lunacode sandbox list / diff / merge / clean` CLI で手元からの操作に対応
- **セキュリティ** — RBAC、危険コマンド検出、SSRF 対策、Undercover モード
...
# サンドボックス（Tier 1 隔離 workspace の操作）
lunacode sandbox list                        # workspace 一覧
lunacode sandbox diff <taskId>               # origin との差分表示
lunacode sandbox merge <taskId> --apply      # workspace の変更を origin に反映
lunacode sandbox merge <taskId> --apply --on-conflict skip-conflicted
lunacode sandbox clean <taskId>              # workspace を削除

# マルチエージェント並列実行（Phase 31）
lunacode parallel "fix bug A" "fix bug B" --max-concurrency 2
lunacode parallel "task1" "task2" "task3" --on-conflict skip-conflicted --timeout 120000
lunacode parallel "dry-run check" --dry-run  # 実行せずプランを JSON 出力
...
# テスト
bun test                            # 自動テスト実行（780 pass / 3 todo）
```
...
全 31 フェーズ中 30 完了（Phase 19 bubbletea のみ保留）。
...
| 21    | テスト実行ツール（SWE-bench 対応）                                                          | 30 pass  |
| 22    | モデル設定レジストリ（Aider 方式）                                                          | —        |
| 23    | post-write 構文チェック                                                                     | —        |
| 24    | サンドボックス階層（Tier 1 作業ツリー）                                                     | 20 pass  |
| 25    | Tier 1 堅牢化（mtime fast path 削除、reflink プローブ外出し、git-worktree ランダム suffix） | +9 pass  |
| 26    | `lunacode sandbox` CLI（list / diff / merge / clean、`--apply` / `--on-conflict`）          | +8 pass  |
| 27    | excludePatterns の glob 化 + `.gitignore` 連携、旧 `SandboxEnvironment` 削除                | +10 pass |
| 28    | auto-merge 衝突検知（baseline + `onConflict: abort \| skip-conflicted \| force`）           | +11 pass |
| 29    | `ToolContext.basePath` 注入、`chdirOnActivate: false` 既定化                                | +9 pass  |
| 30    | パフォーマンス改善 Wave 1–3（ファイル I/O、ベクトル検索、ロガー）                           | +5 pass  |
| 31    | マルチエージェント並列実行（`ParallelAgentCoordinator` + `lunacode parallel` CLI）          | +23 pass |

## テスト

```bash
# 全テスト実行
bun test  # 780 pass / 3 todo / 0 fail / 1760 expect()
...
bun test tests/test-runner.test.ts     # テスト実行ツールテスト
```
...
| test-runner.test.ts           | 30       | フレームワーク自動検出、パーサ、バリデーション、SWE-bench シナリオ |
| workspace-isolator.test.ts    | 20       | サンドボックス Tier 1（隔離コピー、diff、merge、strategy 選択）    |
| sandbox-phase25.test.ts       | 9        | Phase 25: mtime fast path / reflink 外出し / worktree 動作検証     |
| cli-sandbox.test.ts           | 8        | Phase 26: `lunacode sandbox` list/diff/merge/clean smoke           |
| sandbox-phase27.test.ts       | 10       | Phase 27: excludePatterns glob 化、`.gitignore` 連携               |
| sandbox-phase28.test.ts       | 11       | Phase 28: baseline 衝突検知、`onConflict` ポリシー3種              |
| sandbox-phase29.test.ts       | 9        | Phase 29: ToolContext.basePath 注入、cwd 非依存                    |
| parallel-coordinator.test.ts  | 8        | Phase 31: ParallelAgentCoordinator 並列実行、merge、timeout、衝突  |
| cli-parallel.test.ts          | 15       | Phase 31: `lunacode parallel` 引数パース + CLI smoke               |
...
| [ロードマップ](./ROADMAP.md)                              | 完了・進行中・計画中の Phase 一覧（公開用）          |
| [開発計画](./docs/inside/plan.md)                         | 全 24 フェーズの詳細・進捗（内部ノート）             |
