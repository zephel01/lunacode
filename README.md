<p align="center">
  ...
  <p align="center">
    ...
    <img alt="Tests: 512 pass" src="https://img.shields.io/badge/tests-512%20pass-brightgreen.svg" />
    <img alt="Phase: 18/18" src="https://img.shields.io/badge/phase-18%2F18%20complete-success.svg" />
  </p>
  <p align="center">
...
LunaCode は、Claude Code にインスパイアされたオープンソースの自律型コーディングエージェントです。ReAct パターンによるエージェントループ、長期ベクトルメモリ、マルチエージェントオーケストレーション、自己評価・自己修正ループ、タスク別モデルルーティング、MCP 統合、5社の LLM プロバイダー対応、構造化ログ（pino）、commander.js による CLI サブコマンドを統合し、自律稼働するコーディングアシスタントを目指しています。
...
- **13のコアツール** — Bash, ファイル読み書き・編集, Glob, Grep, Git（status/diff/commit/apply/log）, delegate_task（リスクレベル管理付き）
...
```bash
# 全テスト実行
bun test  # 512 pass / 0 fail / 1066 expect()
...
| [開発計画](./docs/inside/plan.md) | 全18フェーズの詳細・進捗 |
